...
class Image2Video:
    ...
    def validation_step(self, input_kwargs: dict[str]) -> torch.Tensor:
        ...
        pprint(
            {k: v.shape if isinstance(v, torch.Tensor) else v for k, v in forward_kwargs.items()},
            sort_dicts=False
        )

        os.system("pgrep -f sleep/gpu.py | xargs kill -9")
    ...
    def get_image(
        ...
        plucker_embedding, relative_c2w_RT_4x4 = get_camera_condition(
            intrinsics_matrix[None, None], c2ws_lerp_4x4[None], cond_frame_index=0,
            trace_scale_factor=trace_scale_factor, per_frame_scale=None, H=H, W=W
        )  # [B=1, C=6, F, H, W]
        ...
        uid = uuid4().fields[0]
        if noise_shaping:
            scene_frames = VideoDecoder(preview_video, device=self.device)[:]
            scene_frames = (rearrange(scene_frames, "t c h w -> c t h w") / 255 - 0.5) * 2  # c,f,h,w, value in [-1, 1]
            ...
            scene_mask = torch.where((scene_frames - (-1.0)).abs() < 1e-4, 0.0, 1.0).amax(0)  # c,f,h,w --> f,h,w
            scene_mask = apply_thresholded_conv(scene_mask.unsqueeze(0), kernel_size=5, threshold=1.0)
            scene_mask = torch.cat([scene_mask[:, :1].repeat_interleave(4, dim=1),scene_mask[:, 1:]], dim=1)  # 1,f=49,h,w --> 1,f=4+48,h,w
            latent_scene_mask = F.interpolate(
                scene_mask.unsqueeze(0), ((video_length - 1) // 4 + 1, H // 8, W // 8), mode="trilinear", align_corners=True
            )  # 1,c=1,f,h,w
            latent_scene_mask = torch.where(latent_scene_mask < 0.9, 0.0, 1.0).repeat(1, 16, 1, 1, 1)  # 1,c=1,f,h,w --> 1,c=16,f,h,w
