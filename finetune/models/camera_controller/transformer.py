...
class CogVideoXControlnetXsBlock(nn.Module):
    ...
    def forward(
        ...
        if epipolar_mask_weight is not None:
            ...
            coarse_gate = torch.tanh(self.coarse_gate_linear(norm_hidden_states).float()).to(dtype)
            fine_gate = torch.tanh(self.fine_gate_linear(norm_hidden_states).float()).to(dtype)
        else:
            coarse_gate, fine_gate = None, None
