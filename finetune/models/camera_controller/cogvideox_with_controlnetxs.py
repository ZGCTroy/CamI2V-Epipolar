...
class CogVideoXBlock(nn.Module):
    ...
    def __init__(
        ...
        if IS_FLASH3_AVAILABLE:
            print(f"IS_FLASH3_AVAILABLE: {IS_FLASH3_AVAILABLE}")
...
class CogVideoXTransformer3DModel(ModelMixin, ConfigMixin, PeftAdapterMixin, CacheMixin):
    ...
    def forward(
        ...
        epipolar_mask_weight: Optional[torch.Tensor] = None, # b,fhw,fhw
    ):
        ...
        skip_layer_ratio = controlnetxs.skip_layer_ratio
        ...
        for i, block in enumerate(self.transformer_blocks):
            if torch.is_grad_enabled() and self.gradient_checkpointing:
                ...
                if controlnetxs is not None and (timestep >= camera_condition_start_timestep).sum() and i >= up_down_layer_start_idx and i<= up_down_layer_end_idx and i % skip_layer_ratio == 0:
                    hidden_states_for_controlnetxs = controlnetxs.down_projectors[(i - up_down_layer_start_idx) // skip_layer_ratio](
                    ...
                    hidden_states_for_controlnetxs = torch.utils.checkpoint.checkpoint(
                        create_custom_forward(controlnetxs.transformer.transformer_blocks[(i - up_down_layer_start_idx) // skip_layer_ratio]),
                    ...
                    hidden_states = controlnetxs.up_projectors[(i - up_down_layer_start_idx) // skip_layer_ratio](
                ...
                if controlnetxs is not None and (timestep >= camera_condition_start_timestep).sum() and i >= up_down_layer_start_idx and i<= up_down_layer_end_idx and i % skip_layer_ratio == 0:
                    ...
                    hidden_states_for_controlnetxs = controlnetxs.transformer.transformer_blocks[(i - up_down_layer_start_idx) // skip_layer_ratio](
